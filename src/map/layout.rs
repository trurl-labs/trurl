...
//!
//! `.trurl/.state/layout.json` stores per-node positions and pinned
...
pub(crate) fn load(store_root: &Path) -> LayoutState {
    ...
    match fs::read_to_string(&path) {
        ...
        Err(e) => {
            eprintln!("trurl: failed to read layout.json: {e}");
            LayoutState::default()
        }
    }
}
...
mod tests {
    ...
    fn save_creates_parent_dirs() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path().join("deep").join(".trurl");
